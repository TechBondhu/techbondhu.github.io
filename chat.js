 ...
 * Removed Rasa API calls from left side; integrated rule-based system from ruleflow.js for left side responses.
...
async function sendMessage(side) {
  ...
  if (!message && !selectedFile) return; // যদি কোনো মেসেজ বা ফাইল না থাকে তবে কিছু করো না
  if (message) {
  ...
  if (side === 'left') {
      if (message.includes("এনআইডি") || message.includes("nid")) {
        ...
        await handleFormFlow(message, displayMessage, displayReview, selectedFile);
      }
      if (selectedFile) {
          clearPreview(side); // ফাইল হ্যান্ডেল হওয়ার পর প্রিভিউ ক্লিয়ার করো
      }
  } else {
